...
//        System.setProperty("java.util.logging.SimpleFormatter.format", "[%1$tF %1$tT] [%4$-7s] [%1$tL] %5$s %n");
    ...
    public void start(Stage stage) throws Exception {
        ...
        menuBar.getMenus().addAll(new Menu[]{mainMenu, logMenu});
        ...
        LimitSwitch limitSwitchP = new LimitSwitch(eventBus, null, 224, 0.050);
        LimitSwitch limitSwitch1 = new LimitSwitch(eventBus, null, 225, 0.275);
        LimitSwitch limitSwitch2 = new LimitSwitch(eventBus, null, 226, 0.500);
        LimitSwitch limitSwitch3 = new LimitSwitch(eventBus, null, 227, 0.725);
        LimitSwitch limitSwitch4 = new LimitSwitch(eventBus, null, 228, 0.95);
        LimitSwitch limitSwitchBottom = new LimitSwitch(eventBus, null, 223, 0);
        LimitSwitch limitSwitchTop = new LimitSwitch(eventBus, null, 229, 1);
        ...
        textArea.setText(String.valueOf(Singleton.getInstance().propabilityRate));
        sliderProp.setValue(Singleton.getInstance().propabilityRate);
        noiseCheckBox.setSelected(Singleton.getInstance().isLINK_NOISE);
        sliderProp.valueProperty().addListener((observable, oldValue, newValue) -> {
            Singleton.getInstance().propabilityRate = (Double) newValue;
            textArea.setText(newValue.toString());
        });
        noiseCheckBox.selectedProperty().addListener((observable, oldValue, newValue) -> {
            Singleton.getInstance().isLINK_NOISE = newValue;
        });
        // Only updates UI
